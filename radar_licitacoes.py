#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Radar v3.3 - PNCP, area + bandeira + filtro juridico"""
...
HEADERS = {"User-Agent": "Mozilla/5.0 (RadarLicitacoes/6)", "Accept": "application/json"}
...
def e_servico_juridico(titulo, descricao):
    """True apenas se o edital eh para contratar servicos juridicos."""
    texto = (str(titulo or "") + " " + str(descricao or "")).lower()
    tem_jur = any(t in texto for t in [
...
def analisar(item, titulo, descricao, orgao, uf, municipio, valor):
    ...
    valor_mensal = valor_total / meses if meses > 0 and valor_total else 0
    ...
    custo_log = viagens * custo_viagem + noites * HOTEL_DIARIA
    custo_ef = 0 if tem_reembolso else custo_log
    if not valor_total:
        lucro = None; bandeira = "cinza"
    elif e_evento:
        lucro = valor_total - (0 if tem_reembolso else custo_log)
        bandeira = "verde" if lucro >= LUCRO_VERDE else ("amarelo" if lucro >= LUCRO_AMARELO else "vermelho")
    else:
        lucro = valor_mensal - custo_ef
        bandeira = "verde" if lucro >= LUCRO_VERDE else ("amarelo" if lucro >= LUCRO_AMARELO else "vermelho")
    ...
    if valor_total:
        ...
        p.append("valor nao informado na busca")
    ...
    elif custo_log: p.append(f"logistica ~{fmt_brl(custo_log)}/mes")
    if trofeu: p.append(f"trofeu: {trofeu}")
    if lucro is not None:
        suf = "/mes" if not e_evento else ""
        p.append(f"<b>lucro ~{fmt_brl(lucro)}{suf}</b> (otimista, desagio 20-40%)")
    ...
    for pagina in range(1, PAGINAS_POR_KEYWORD + 1):
        ...
        try:
            ...
            if len(content) > LIMITE_BYTES:
                print(f"  [aviso] grande em '{keyword}'"); break
...
def renderizar_html(items, somente_abertos):
    ...
    titulo_filtro = "editais ABERTOS - servicos juridicos" if somente_abertos else "todos"
    ...
    qlinks_css = ".qlink{display:inline-flex;align-items:center;gap:6px;background:#fff;border:1px solid #d0d5e8;border-radius:18px;padding:6px 13px;font-size:12px;color:#1E2B6A;text-decoration:none;font-weight:600;transition:all .15s}.qlink:hover{background:#1E2B6A;color:#fff;border-color:#1E2B6A}.qlink.current{background:#1E2B6A;color:#fff;border-color:#1E2B6A;cursor:default;pointer-events:none;opacity:.95}.qlink.current::after{content:' \\00B7  voce esta aqui';font-weight:400;font-size:10.5px;opacity:.85}.qlinks-bar{background:#f5f7fc;border-bottom:1px solid #e0e4f0;padding:10px 24px;margin:-24px -24px 18px}.qlinks-inner{max-width:1280px;margin:0 auto;display:flex;gap:8px;align-items:center;flex-wrap:wrap}"
    qlinks_html = '<div class="qlinks-bar"><div class="qlinks-inner"><div style="font-size:10.5px;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:.06em;margin-right:6px">🌐 Sites do escritorio:</div><a class="qlink" href="https://rodsoares88.github.io/mesa-executiva-ns/" target="_blank" rel="noopener" title="Mesa Executiva NS - uso restrito socios">🏛️ Mesa Executiva NS</a><a class="qlink" href="https://rodsoares88.github.io/nepomucenoaores-bmg/" target="_blank" rel="noopener" title="Inteligencia BMG · decisoes, modelos, ranking">⚖️ Inteligencia BMG</a><a class="qlink" href="https://rodsoares88.github.io/painel-interno/" target="_blank" rel="noopener" title="Central de Comando · encerramentos, financeiro">📊 Central de Comando</a><a class="qlink current" href="#" title="Radar Licitacoes - voce esta aqui">🎯 Radar Licitacoes</a><a class="qlink" href="https://rodsoares88.github.io/radar-analise/" target="_blank" rel="noopener" title="Analise tecnica de editais">🔬 Radar Analise</a><span style="font-size:10.5px;color:#999;margin-left:8px">↗ abrem em nova aba</span></div></div>'

    return f"""<!DOCTYPE html><html lang="pt-BR"><head><meta charset="utf-8"><title>Radar de Licitacoes</title><meta name="viewport" content="width=device-width,initial-scale=1"><style>{css}{qlinks_css}</style></head><body>{qlinks_html}<header><div><h1>📡 Radar de Licitacoes</h1><div class="sub">Atualizado: {agora} - PNCP - {titulo_filtro}</div></div><div class="stats"><div class="stat"><div class="num">{total}</div><div class="lbl">Total</div></div>{blocos_esfera}{blocos_band}</div></header><div class="filters"><label>🔍 Buscar: <input type="text" id="filter-text" placeholder="palavra, orgao..."></label><label>Bandeira: <select id="filter-band"><option value="">Todas</option><option value="verde">🟢 Verde</option><option value="amarelo">🟡 Amarelo</option><option value="vermelho">🔴 Vermelho</option><option value="cinza">⚪ Sem dados</option></select></label><label>Area: <select id="filter-area"><option value="">Todas</option>{opcoes_areas}</select></label><label>Esfera: <select id="filter-esfera"><option value="">Todas</option><option value="Federal">Federal</option><option value="Estadual">Estadual</option><option value="Municipal">Municipal</option><option value="Distrital">Distrital</option></select></label><label>UF: <select id="filter-uf"><option value="">Todas</option></select></label></div><table><thead><tr><th>Esfera</th><th>Objeto / Orgao</th><th>Modalidade</th><th>Valor</th><th>Datas</th><th>Analise</th><th>Acao</th></tr></thead><tbody>{linhas_html}</tbody></table><script>{js}</script></body></html>"""
...
def main():
    ...
    OUTPUT_JSON.write_text(json.dumps({
        ...
        "filtro": "recebendo_proposta + juridico" if somente_abertos else "todos",
