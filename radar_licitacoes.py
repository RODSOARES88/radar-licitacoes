#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Radar v4.2 - PNCP completo final: area, bandeira, filtro juridico permissivo, IA"""
...
HEADERS = {"User-Agent": "Mozilla/5.0 (RadarLicitacoes/9)", "Accept": "application/json"}
...
def e_servico_juridico(titulo, descricao):
    """Permissivo: aceita varios indicios de servico juridico, rejeita so claramente nao-juridicos."""
    texto = (str(titulo or "") + " " + str(descricao or "")).lower()
    tem_jur = any(t in texto for t in [
        "advocac","advogad","juridic","jurídic","judic",
        "consultoria tribut","consultoria juridic","consultoria fiscal","consultoria contabil","consultoria empresarial",
        "assessoria tribut","assessoria juridic","assessoria fiscal","assessoria contabil","assessoria empresarial",
        "servicos advocat","serviços advocat","servicos juridic","serviços juridic",
        "escritorio de advocac","escritório de advocac","sociedade de advogad","sociedade individual de advoca",
        "contencioso","patrocinio","patrocínio","representacao judicial","representação judicial",
        "parecer","pareceres","execucao fiscal","execução fiscal",
        "recuperacao de credit","recuperação de crédit","defesa em proce","defesa judicial",
        "consultoria especializa",
    ])
    if not tem_jur: return False
    negativos = ["veiculos automotores","veículos automotores","merenda escolar","transporte escolar",
                 "obras de construcao","obras de construção","uniformes escolares","fardamento militar",
                 "manutencao predial","manutenção predial","limpeza e conservacao","limpeza e conservação",
                 "papelaria e suprim"]
...
def renderizar_html(items, somente_abertos):
    ...
    for it in items_norm:
        ...
        linhas.append(linha)
    linhas_html = "".join(linhas) if linhas else '<tr><td colspan="7" class="empty">Nenhum edital encontrado.</td></tr>'
