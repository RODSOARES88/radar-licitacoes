#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Radar v3.1 - PNCP, classificacao por area + bandeira de viabilidade"""
...
HEADERS = {"User-Agent": "Mozilla/5.0 (RadarLicitacoes/5.1)", "Accept": "application/json"}
...
def analisar(item, titulo, descricao, orgao, uf, municipio, valor):
    ...
    else: p.append("valor nao informado")
    ...
    if lucro is not None:
        ...
        p.append(f"<b>lucro liquido ~{fmt_brl(lucro)}{suf}</b>")
    resumo = ". ".join(p) + "."
...
def renderizar_html(items, somente_abertos):
    items_norm = sorted([normalizar(it) for it in items],
        key=lambda x: (0 if x["bandeira"] == "verde" else (1 if x["bandeira"] == "amarelo" else 2),
                       -(x["lucro"] or -1e9)))
    ...
    return f"""<!DOCTYPE html><html lang="pt-BR"><head><meta charset="utf-8"><title>Radar de Licitacoes</title><meta name="viewport" content="width=device-width,initial-scale=1"><style>{css}</style></head><body><header><div><h1>📡 Radar de Licitacoes</h1><div class="sub">Atualizado: {agora} - PNCP - {titulo_filtro}</div></div><div class="stats"><div class="stat"><div class="num">{total}</div><div class="lbl">Total</div></div>{blocos_esfera}{blocos_band}</div></header><div class="filters"><label>🔍 Buscar: <input type="text" id="filter-text" placeholder="palavra, orgao..."></label><label>Bandeira: <select id="filter-band"><option value="">Todas</option><option value="verde">🟢 Verde</option><option value="amarelo">🟡 Amarelo</option><option value="vermelho">🔴 Vermelho</option></select></label><label>Area: <select id="filter-area"><option value="">Todas</option>{opcoes_areas}</select></label><label>Esfera: <select id="filter-esfera"><option value="">Todas</option><option value="Federal">Federal</option><option value="Estadual">Estadual</option><option value="Municipal">Municipal</option><option value="Distrital">Distrital</option></select></label><label>UF: <select id="filter-uf"><option value="">Todas</option></select></label></div><table><thead><tr><th>Esfera</th><th>Objeto / Orgao</th><th>Modalidade</th><th>Valor</th><th>Datas</th><th>Analise</th><th>Acao</th></tr></thead><tbody>{linhas_html}</tbody></table><script>{js}</script></body></html>"""
